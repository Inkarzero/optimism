...
func (s *AltDADataSource) Next(ctx context.Context) (eth.Data, error) {
	...
	if err := s.fetcher.AdvanceL1Origin(ctx, s.l1, s.id.ID()); err != nil {
		if errors.Is(err, altda.ErrReorgRequired) {
			s.log.Debug("optimism/op-node/rollup/derive/altda_data_source.go | Next | stopped, altda.ErrReorgRequired (new expired challenge)")
	...
	if s.comm == nil {
		...
		if err != nil {
			s.log.Debug("optimism/op-node/rollup/derive/altda_data_source.go | Next | stopped, err in src.Next", "err", err)
			return nil, err
		}
		if len(data) == 0 {
			s.log.Debug("optimism/op-node/rollup/derive/altda_data_source.go | Next | stopped, len(data) == 0")
		...
		if data[0] != altda.TxDataVersion1 {
			s.log.Debug("optimism/op-node/rollup/derive/altda_data_source.go | Next | stopped, tx data type is not altDA")
			return data, nil
		}
	...
	if errors.Is(err, altda.ErrReorgRequired) {
		s.log.Debug("optimism/op-node/rollup/derive/altda_data_source.go | Next | stopped, altda.ErrReorgRequired")
		...
		s.log.Debug("optimism/op-node/rollup/derive/altda_data_source.go | Next | stopped, altda.ErrExpiredChallenge")
		...
		s.log.Debug("optimism/op-node/rollup/derive/altda_data_source.go | Next | stopped, altda.ErrMissingPastWindow")
		...
		s.log.Debug("optimism/op-node/rollup/derive/altda_data_source.go | Next | stopped, altda.ErrPendingChallenge")
		// continue stepping without slowing down.
		return nil, NotEnoughData
	} else if err != nil {
		// return temporary error so we can keep retrying.
		s.log.Debug("optimism/op-node/rollup/derive/altda_data_source.go | Next | stopped, some unrecognized error in GetInput", "err", err)
