...
func (fi *Finalizer) tryFinalize() {
	...
	for _, fd := range fi.finalityData {
		if fd.L2Block.Number > finalizedL2.Number && fd.L1Block.Number <= fi.finalizedL1.Number {
			fi.log.Debug("op-node/rollup/finality/finalizer.go | tryFinalize | found L2 block to finalize ", "oldFinalizedL2.Number", finalizedL2.Number, "newFinalizedL2.Number", fd.L2Block.Number, "fd.L1Block.Number", fd.L1Block.Number)
