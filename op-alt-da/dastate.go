...
func (s *State) ClearCommitments() {
	s.log.Debug("optimism/op-alt-da/dastate.go | ClearCommitments | started", "len(s.commitments)", len(s.commitments), "len(s.expiredCommitments)", len(s.expiredCommitments))
...
func (s *State) Reset() {
	s.log.Debug("optimism/op-alt-da/dastate.go | Reset | started", "len(s.commitments)", len(s.commitments), "len(s.expiredCommitments)", len(s.expiredCommitments), "len(s.challenges)", len(s.challenges), "len(s.expiredChallenges)", len(s.expiredChallenges))
	s.commitments = s.commitments[:0]
	s.expiredCommitments = s.expiredCommitments[:0]
...
func (s *State) CreateChallenge(comm CommitmentData, inclusionBlock eth.BlockID, commBlockNumber uint64) {
	s.log.Debug("optimism/op-alt-da/dastate.go | CreateChallenge | started", "comm", comm, "inclusionBlock", inclusionBlock, "commBlockNumber", commBlockNumber)
	c := &Challenge{
		commData:                 comm,
...
func (s *State) ResolveChallenge(comm CommitmentData, inclusionBlock eth.BlockID, commBlockNumber uint64, input []byte) error {
	s.log.Debug("optimism/op-alt-da/dastate.go | ResolveChallenge | started", "comm", comm, "inclusionBlock", inclusionBlock, "commBlockNumber", commBlockNumber, "input", input)
	c, ok := s.challengesMap[challengeKey(comm, commBlockNumber)]
	if !ok {
		s.log.Debug("optimism/op-alt-da/dastate.go | ResolveChallenge | stopped, challenge was not tracked")
		return errors.New("challenge was not tracked")
	}
	c.input = input
	c.challengeStatus = ChallengeResolved
	s.log.Debug("optimism/op-alt-da/dastate.go | ResolveChallenge | finished, challenge resolved")
	return nil
}
...
func (s *State) TrackCommitment(comm CommitmentData, inclusionBlock eth.L1BlockRef) {
	s.log.Debug("optimism/op-alt-da/dastate.go | TrackCommitment | started", "comm", comm, "inclusionBlock", inclusionBlock)
	c := Commitment{
		data:               comm,
...
func (s *State) GetChallenge(comm CommitmentData, commBlockNumber uint64) (*Challenge, bool) {
	s.log.Debug("optimism/op-alt-da/dastate.go | GetChallenge | started", "comm", comm, "commBlockNumber", commBlockNumber)
	challenge, ok := s.challengesMap[challengeKey(comm, commBlockNumber)]
	s.log.Debug("optimism/op-alt-da/dastate.go | GetChallenge | finished", "challenge", challenge, "ok", ok)
	return challenge, ok
}
...
func (s *State) GetChallengeStatus(comm CommitmentData, commBlockNumber uint64) ChallengeStatus {
	s.log.Debug("optimism/op-alt-da/dastate.go | GetChallengeStatus | started", "comm", comm, "commBlockNumber", commBlockNumber)
	challenge, ok := s.GetChallenge(comm, commBlockNumber)
	if ok {
		s.log.Debug("optimism/op-alt-da/dastate.go | GetChallengeStatus | finished, ok", "challenge", challenge)
		return challenge.challengeStatus
	}
	s.log.Debug("optimism/op-alt-da/dastate.go | GetChallengeStatus | finished, not ok", "challenge", challenge)
...
func (s *State) NoCommitments() bool {
	s.log.Debug("optimism/op-alt-da/dastate.go | NoCommitments | started", "len(s.challenges)", len(s.challenges), "len(s.expiredChallenges)", len(s.expiredChallenges), "len(s.commitments)", len(s.commitments), "len(s.expiredCommitments)", len(s.expiredCommitments))
...
func (s *State) ExpireCommitments(origin eth.BlockID) error {
	s.log.Debug("optimism/op-alt-da/dastate.go | ExpireCommitments | started", "origin", origin, "len(s.commitments)", len(s.commitments))
	var err error
	for len(s.commitments) > 0 {
		...
		if expiresAt > origin.Number {
			s.log.Debug("optimism/op-alt-da/dastate.go | ExpireCommitments | stopped, commitment expires the in future", "expiresAt", expiresAt, "origin.Number", origin.Number)
			return err
		}
	...
	s.log.Debug("optimism/op-alt-da/dastate.go | ExpireCommitments | finished", "err", err)
	return err
}
...
func (s *State) ExpireChallenges(origin eth.BlockID) {
	s.log.Debug("optimism/op-alt-da/dastate.go | ExpireChallenges | started", "origin", origin, "len(s.challenges)", len(s.challenges))
...
func (s *State) Prune(origin eth.BlockID) {
	s.log.Debug("optimism/op-alt-da/dastate.go | Prune | started", "origin", origin)
...
func (s *State) pruneCommitments(origin eth.BlockID) {
	s.log.Debug("optimism/op-alt-da/dastate.go | pruneCommitments | started", "origin", origin, "len(s.expiredCommitments)", len(s.expiredCommitments), "s.expiredCommitments", s.expiredCommitments)
	for len(s.expiredCommitments) > 0 {
		...
		s.log.Debug("optimism/op-alt-da/dastate.go | pruneCommitments | finished", "s.lastPrunedCommitment", s.lastPrunedCommitment)
	}
}
